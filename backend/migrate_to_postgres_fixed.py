"""
SQLite 到 PostgreSQL 数据迁移脚本 (已修复)

修复内容:
1. 更新导入语句 (Folder替代Category)
2. 添加folders表迁移
3. 处理category_id到folder_id的数据迁移
4. 更新验证列表

使用方法:
    python migrate_to_postgres_fixed.py --sqlite-path /path/to/database.db
    python migrate_to_postgres_fixed.py --skip-verify  # 跳过验证
...
def check_sqlite_schema(sqlite_engine):
    """检查SQLite数据库的实际schema"""
    print("\n[STEP] 检查SQLite数据库schema...")
    ...
    with sqlite_engine.connect() as conn:
        ...
        has_folders = result.fetchone() is not None

        # 检查categories表是否存在
        result = conn.execute(text(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='categories'"
        ))
        has_categories = result.fetchone() is not None

        # 检查scripts表字段
        result = conn.execute(text("PRAGMA table_info(scripts)"))
        script_columns = [col[1] for col in result.fetchall()]

        has_folder_id = 'folder_id' in script_columns
        has_category_id = 'category_id' in script_columns

    print(f"[INFO] SQLite数据库状态:")
    print(f"  - folders表: {'存在' if has_folders else '不存在'}")
    print(f"  - categories表: {'存在' if has_categories else '不存在'}")
    print(f"  - scripts.folder_id: {'存在' if has_folder_id else '不存在'}")
    print(f"  - scripts.category_id: {'存在' if has_category_id else '不存在'}")
...
def migrate_categories_to_folders(sqlite_engine, pg_engine):
    """
    如果SQLite中categories有数据但folders无数据，
    将categories数据迁移到folders并建立映射
    """
    print("\n[STEP] 处理categories到folders的数据迁移...")

    # 检查数据
    with sqlite_engine.connect() as conn:
        folders_count = get_table_count(sqlite_engine, 'folders')
        categories_count = get_table_count(sqlite_engine, 'categories')

        print(f"[INFO] folders表: {folders_count} 条数据")
        print(f"[INFO] categories表: {categories_count} 条数据")

        # 如果folders已有数据，直接返回
        if folders_count > 0:
            print("[INFO] folders表已有数据，跳过categories迁移")
            return None

        # 如果categories无数据，直接返回
        if categories_count == 0:
            print("[INFO] categories表无数据，跳过迁移")
            return None

        # 迁移categories数据到folders
        print("[INFO] 开始迁移categories数据到folders...")

        result = conn.execute(text(
            "SELECT id, name, color, icon, sort_order, created_at, updated_at FROM categories"
        ))
        categories = result.fetchall()

        # 建立映射：category_id -> folder_id
        mapping = {}
        ...
        with pg_engine.connect() as pg_conn:
            for category in categories:
                ...
                # 插入到folders表（parent_id=NULL，扁平结构）
                insert_result = pg_conn.execute(text(
                    """
                    INSERT INTO folders (name, parent_id, color, sort_order, created_at, updated_at)
                    VALUES (:name, NULL, :color, :sort_order, :created_at, :updated_at)
                    RETURNING id
                    """
                ), {
                    'name': name,
                    'color': color or '#E6A23C',
                    'sort_order': sort_order or 0,
                    'created_at': created_at or datetime.utcnow(),
                    'updated_at': updated_at or datetime.utcnow()
                })

                new_folder_id = insert_result.scalar()
                mapping[cat_id] = new_folder_id

            pg_conn.commit()

        print(f"[OK] 成功迁移 {len(mapping)} 条categories数据到folders")
        print(f"[INFO] 建立了映射关系: {len(mapping)} 个")

        return mapping


def update_script_folder_id(sqlite_engine, pg_engine, mapping):
    """
    如果SQLite中scripts使用category_id，需要更新到folder_id
    """
    if not mapping:
        print("\n[INFO] 无需更新script.folder_id映射")
        return

    print("\n[STEP] 更新scripts的folder_id...")

    # 检查scripts表是否有category_id字段
    with sqlite_engine.connect() as conn:
        result = conn.execute(text("PRAGMA table_info(scripts)"))
        columns = [col[1] for col in result.fetchall()]

        if 'category_id' not in columns:
            print("[INFO] scripts表无category_id字段，跳过")
            return

        # 获取使用category_id的scripts
        result = conn.execute(text(
            "SELECT id, category_id FROM scripts WHERE category_id IS NOT NULL"
        ))
        scripts_to_update = result.fetchall()

        if not scripts_to_update:
            print("[INFO] 无scripts使用category_id，跳过")
            return

        print(f"[INFO] 找到 {len(scripts_to_update)} 个脚本需要更新folder_id")

        # 更新folder_id
        updated_count = 0
        with pg_engine.connect() as pg_conn:
            for script_id, category_id in scripts_to_update:
                if category_id in mapping:
                    folder_id = mapping[category_id]
                    pg_conn.execute(text(
                        "UPDATE scripts SET folder_id = :folder_id WHERE id = :script_id"
                    ), {'folder_id': folder_id, 'script_id': script_id})
                    updated_count += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='SQLite 到 PostgreSQL 数据迁移脚本 (已修复)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
    python migrate_to_postgres_fixed.py
    python migrate_to_postgres_fixed.py --sqlite-path /path/to/database.db
    python migrate_to_postgres_fixed.py --skip-verify
...
环境变量 (PostgreSQL 连接配置):
    ...
    print("=" * 60)
    print("SQLite 到 PostgreSQL 数据迁移 (修复版)")
    print("=" * 60)
    ...
    # 检查SQLite schema
    schema_info = check_sqlite_schema(sqlite_engine)
    ...
    tables_order = [
        # (表名, 模型类, JSON字段列表)
        ('folders', Folder, None),  # ← 替换categories，使用Folder模型
        ('tags', Tag, None),
    ...
    # 如果SQLite中有categories数据但folders为空，先迁移categories到folders
    category_folder_mapping = None
    if schema_info['has_categories'] and not schema_info['has_folders']:
        category_folder_mapping = migrate_categories_to_folders(sqlite_engine, pg_engine)
    ...
    for item in tables_order:
        ...
        # 如果是folders表且已通过migrate_categories_to_folders处理，跳过
        if table_name == 'folders' and category_folder_mapping:
            results[table_name] = len(category_folder_mapping)
            continue
    ...
    # 如果有category→folder映射，更新scripts的folder_id
    if category_folder_mapping and schema_info['has_category_id']:
        update_script_folder_id(sqlite_engine, pg_engine, category_folder_mapping)
